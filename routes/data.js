    ...
    redditCount.find({tag: q}, function (err, docs){
        if(err){
            ...
            if(req.query.more && req.query.more == 1){
                ...
                options = {
                    'method': 'GET',
                    'url': `http://localhost:5000/reddit/search/?q=${q}&sort=hot&limit=${limit+5}`  //Change to docker name
                };
                request(options, function (error, response) {
                    if (error){
                        throw new Error(error)
                ...
                options = {
                    'method': 'GET',
                    'url': `http://localhost:5000/reddit/search/?q=${q}&sort=hot&limit=${limit}`  //Change to docker name
                };
                request(options, function (error, response) {
                    if (error){
                        throw new Error(error)
